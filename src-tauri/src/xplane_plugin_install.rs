//! ASA-ACARS X-Plane Premium Plugin auto-install (v0.5.0+).
//!
...
//!     `ASA-ACARS-XPlane-Plugin-vX.Y.Z.zip` from this release and
//!     extracts it to `<install_dir>/Resources/plugins/ASA-ACARS/`.
...
//!      `Resources/plugins/`).
//!   2. Creates the ASA-ACARS subfolder.
...
const PLUGIN_ZIP_URL_TEMPLATE: &str =
    "https://github.com/oAlvesOFF/ASA-Acars/releases/download/v{VERSION}/ASA-ACARS-XPlane-Plugin-v{VERSION}.zip";
...
/// plugins/ASA-ACARS/`. Returns the absolute path of the resulting
/// folder + counts.
pub async fn install_plugin(xplane_root: &Path) -> Result<PluginInstallResult, String> {
    ...
    tracing::info!(url = %url, "downloading ASA-ACARS X-Plane plugin zip");
    ...
    if !resp.status().is_success() {
        return Err(format!(
            "GitHub returned status {} when fetching plugin zip from {}. \
             Make sure this ASA-ACARS version has a matching release.",
            resp.status(),
            url
        ));
    }
    ...
    // ---- Extract ----
    // The zip's top-level entry is `ASA-ACARS/...`. We strip that
    // prefix so files land directly under the user's chosen
    // `<x-plane>/Resources/plugins/ASA-ACARS/` folder.
    ...
    for i in 0..archive.len() {
        ...
        // Strip the leading `ASA-ACARS/` if present so we don't end
        // up at `<...>/ASA-ACARS/ASA-ACARS/64/win.xpl`.
        let stripped = entry_path
            .strip_prefix("ASA-ACARS")
