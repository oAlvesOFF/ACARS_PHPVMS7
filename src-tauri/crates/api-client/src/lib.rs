...
//!   * ASA-ACARS phpVMS module (config, version, heartbeat, landing extras) — Phase 4
//!
//! Authentication: phpVMS API key sent via the `X-API-Key` header (phpVMS standard).
//! All requests advertise `User-Agent: ASA-ACARS/<version>` so the server can identify us.
...
impl Client {
    pub fn new(conn: Connection) -> Result<Self, ApiError> {
        let user_agent = format!("ASA-ACARS/{}", env!("CARGO_PKG_VERSION"));
