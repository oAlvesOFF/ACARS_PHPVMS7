...
//!
//! When the optional **ASA-ACARS X-Plane Plugin** (v0.5.3+) is installed
...
///     for descent (matches ASA-ACARS convention).
...
impl PremiumListener {
    ...
    /// ASA-ACARS instance is already bound). The adapter logs the
    ...
    if env.v != 1 {
        tracing::warn!(
            ...
            "premium: unsupported schema version (expected 1) — dropping packet. \
             ASA-ACARS client may need an update."
        );
        return;
    }
