...
pub struct XPlaneAdapter {
    ...
    /// Listener for the optional ASA-ACARS X-Plane Plugin (v0.5.0+).
...
impl XPlaneAdapter {
    ...
    /// Status of the ASA-ACARS X-Plane Plugin connection (v0.5.0+).
...
fn run_listener(shared: Arc<AdapterShared>) {
    ...
    // so we recover from:
    //   * ASA-ACARS started before X-Plane (initial subscribe lands on
    ...
    while !shared.stop.load(Ordering::SeqCst) {
        ...
        // subscription set. This covers the "ASA-ACARS started before
