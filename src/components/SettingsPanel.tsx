...
export function SettingsPanel({
  ...
  // Drei Gruppen:
  //   - required: SimBrief + Simulator + Filing → was ASA-ACARS zwingend
  ...
  const [activeTab, setActiveTab] = useState<SettingsTab>(() => {
    try {
      const saved = localStorage.getItem("asa-acars.settings.activeTab");
      if (
        saved === "simulator" ||
        saved === "required" ||
        saved === "extras" ||
  ...
  const switchTab = (next: SettingsTab) => {
    setActiveTab(next);
    try {
      localStorage.setItem("asa-acars.settings.activeTab", next);
    } catch {
      /* noop */
    }
  };
  ...
  return (
    <section className="settings">
      ...
      {/* ─── Tab: Simulator ────────────────────────────────────────
          ...
          falsch oder nicht gesetzt, hilft ASA-ACARS nicht. Default-
      ...
      */}
      ...
      {/* ─── Tab: Plugins ─────────────────────────────────────────
          Flugzeug-spezifische Add-ons (PMDG SDK, ASA-ACARS-X-Plane-
 ...
 *
 * Persistence: localStorage `asa-acars.errorReporting.enabled`
