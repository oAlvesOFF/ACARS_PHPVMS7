 ...
 * **localStorage-Keys:**
 *
 * - `asa-acars.update.first_seen.{version}` — wann das aktuelle Update
 *   zum ersten Mal entdeckt wurde (für Stage-Berechnung)
 * - `asa-acars.update.dismissed_until` — Pilot klickt „Später" → 4 h
 *   Banner-Stille (Button bleibt sichtbar)
 * - `asa-acars.update.last_check_at` — letzter erfolgreicher Check
...
function firstSeenKey(version: string): string {
  return `asa-acars.update.first_seen.${version}`;
}

const DISMISS_KEY = "asa-acars.update.dismissed_until";
const LAST_CHECK_KEY = "asa-acars.update.last_check_at";
...
function pruneOldVersionKeys(currentVersion: string) {
  try {
    const prefix = "asa-acars.update.first_seen.";
